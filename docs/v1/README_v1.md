> **Archived.** This is the project README as of v0.1 (September 2026, before the NQX-S1 chip was
> signed off). It is kept for history; the current overview is the [root README](../../README.md).
> Commands in it refer to files now in [`reference/`](../../reference/) and [`labs/`](../../labs/).
...
Experimental drop-in alternatives live in [`plan_b/`](../../reference/plan_b/) — `quasicrystal.py`, `golden_jl.py`, `phinary.py`, `fractal_hash.py`, `groq_dataflow.py`, `multimodal_spiral.py`. Untested, marked experimental.
