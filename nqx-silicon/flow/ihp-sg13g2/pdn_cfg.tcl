# NQX-S1 PDN, derived from the LibreLane 3.0.14 default pdn_cfg.tcl.
#
...
# NQX-S1: the I/O supply nets (IOVDD/IOVSS) only live in the pad ring, where
# the pads connect by abutment. Create them so the pad pins can be globally
# connected, but keep them out of the core voltage domain so no core ring or
# straps are built for them.
set secondary []
foreach net_name {IOVDD IOVSS} sig_type {POWER GROUND} {
    if {[[ord::get_db_block] findNet $net_name] == "NULL"} {
        set net [odb::dbNet_create [ord::get_db_block] $net_name]
        $net setSpecial
