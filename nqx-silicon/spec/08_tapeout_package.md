...
| NQX-S1 readiness | **Ready**: `flow/wafer-space-gf180`, CI builds GDS and runs the wafer.space precheck | **Ready**: signed-off GDS in `tapeout/ihp-sg13g2` | Needs a smaller variant (see §5) | Needs a Caravel wrapper port (not done) |
...
3. Tiny Tapeout only for a demo, with a reduced configuration.
...
onto Tiny Tapeout's `ui_in` / `uo_out` / `uio` pins. A TT submission needs
the TT template repository, a `tt_um_*` wrapper and `info.yaml`; these are
not included yet.
